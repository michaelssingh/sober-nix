...
# Option to enable/disable forced TLS
weechat.config_register_plugin("force_tls", "false", "boolean", "force_tls", "Force TLS for new networks", "force_tls_cb", "", "false")
...
def force_tls_cb(data, option, value):
...
def handle_bouncer_msg(data, signal, signal_data):
    ...
    # Check if TLS should be forced
    force_tls = weechat.config_get_plugin("force_tls")
    if weechat.config_string(force_tls) == "true":
        if weechat_version >= 0x04000000:
