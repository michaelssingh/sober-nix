...
in
{
  ...
  home.activation.provision-soju = lib.hm.dag.entryAfter ["writeBoundary"] ''
    ...
    ${pkgs.soju}/bin/sojuctl -config ${config.home.homeDirectory}/.config/soju/config user run init network update -name libera -username "init" -pass "dT4d8y3Tz*kavNrmue4YzDsX3^VdU%9UA%8U" -sasl plain || true
