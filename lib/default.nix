...

{
  ...
  let
    ...
    shellBinName = if pkgs.lib.hasPrefix "bash" (shellPkg.pname or "") then "bash" else (shellPkg.pname or "sh");
    ...
    passwdContent = ''
      ...
      ${pkgs.lib.concatStrings (pkgs.lib.mapAttrsToList (uName: u:
        "${uName}:x:${toString u.uid}:${toString u.gid}:${u.description or uName}:${u.home or "/var/empty"}:${u.shell or shellPath}\n"
      ) users)}'';

    userGroups = pkgs.lib.mapAttrs' (username: u:
      pkgs.lib.nameValuePair username { gid = u.gid; }
    ) users;
    ...
    groupContent = ''
      root:x:0:
      ${pkgs.lib.concatStrings (pkgs.lib.mapAttrsToList (gName: g:
        "${gName}:x:${toString g.gid}:\n"
      ) allGroups)}'';
    ...
    standardEnv = {
      PATH = pkgs.lib.makeBinPath (
        baseContents ++ packages ++ [ entrypointScript ] ++ (if usrBinEnv && harden then [ pkgs.busybox ] else [])
      );
  ...
  pkgs.dockerTools.buildLayeredImage {
    ...
    contents = baseContents ++ [ envLayer entrypointScript ] ++ packages ++ extraContents;
