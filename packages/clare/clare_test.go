...
func TestDisabledProvidersConfig(t *testing.T) {
	cfg := Config{
		DisabledProviders: []string{"vidsrc"},
	}
	if cfg.IsProviderEnabled("vidsrc") {
		t.Fatalf("expected vidsrc to be disabled")
	}
	if !cfg.IsProviderEnabled("anidb") {
		t.Fatalf("expected anidb to be enabled")
	}
}
...
func TestProviderVidSrc(t *testing.T) {
	...
	if err != nil || len(tvEps) == 0 {
		t.Fatalf("vidsrc TV fetch episodes failed: %v", err)
	}
	t.Logf("✓ VidSrc TV show: %s, total episodes: %d (S01E01..%s)", tvShow.Name, len(tvEps), tvEps[len(tvEps)-1])

	tvStreams, err := p.ResolveStreams("vidsrc:tv:2190", "sub", "S02E01", "best")
	if err != nil || len(tvStreams) == 0 {
		t.Fatalf("vidsrc TV S02E01 resolve streams failed: %v", err)
	}
...
func TestAniDBInvalidIDRejection(t *testing.T) {
	...
	if _, _, err := p.FetchEpisodes("vidsrc:3pCcrhBgTN5vFcCsh", "sub"); err == nil {
		t.Errorf("expected error when passing vidsrc ID to AniDB provider")
	}

	if _, err := p.ResolveStreams("vidsrc:3pCcrhBgTN5vFcCsh", "sub", "1", "best"); err == nil {
		t.Errorf("expected error when resolving streams for vidsrc ID on AniDB provider")
	}
